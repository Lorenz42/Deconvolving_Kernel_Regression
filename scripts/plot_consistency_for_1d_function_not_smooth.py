...
def test_function(x_values):
    return np.abs(x_values-2)-np.abs(x_values +1)+np.abs(x_values+3)   
...
for ax, n_samples, solution_epanechnikov, solution_deconvolving, true_solution, convolved_function in zip(
    ...
    ax.plot(query_points, true_solution, label=r"$f(x)=|x-2|- |x+1|+|x+3|$",
...
output_path = os.path.join(output_folder, "plot_consistency_not_smooth.pdf")
